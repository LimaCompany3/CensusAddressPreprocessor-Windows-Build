...
public static class DLL_703854028M
{
    ...
    public static void NormalizeAddress(SqlString rawAddress, SqlString postalCode,
        ...
        out SqlString fraction,out SqlString streetCore,out SqlString tokenPrefixKey,out SqlString tokenPhoneticKey,
        out SqlString sortedTokenPrefixKey,out SqlString sortedTokenPhoneticKey,out SqlString postalCodeOut)
    {
        streetName=relaxedStreet=numericKey=streetSoundex=houseNumber=alphaSuffix=fraction=streetCore=tokenPrefixKey=tokenPhoneticKey=sortedTokenPrefixKey=sortedTokenPhoneticKey=postalCodeOut=SqlString.Null;
        ...
        try {
            ...
            Match ha=Regex.Match(hn,@"^(\d+)([A-Z]+)-(\d+)$");
            if(hp.Success){primaryNumber=new SqlInt32(Int32.Parse(hp.Groups[1].Value));if(hp.Groups[2].Success)secondaryNumber=new SqlInt32(Int32.Parse(hp.Groups[2].Value));if(hp.Groups[3].Success)alphaSuffix=new SqlString(hp.Groups[3].Value);if(hp.Groups[4].Success)fraction=new SqlString(ReduceFraction(hp.Groups[4].Value));}
            else if(ha.Success){primaryNumber=new SqlInt32(Int32.Parse(ha.Groups[1].Value));alphaSuffix=new SqlString(ha.Groups[2].Value);secondaryNumber=new SqlInt32(Int32.Parse(ha.Groups[3].Value));}
            ...
            List<string> pk=new List<string>(),ph=new List<string>(); foreach(string x in w){pk.Add(x.Substring(0,Math.Min(3,x.Length)));ph.Add(Soundex(x));} tokenPrefixKey=new SqlString(String.Join("|",pk));tokenPhoneticKey=new SqlString(String.Join("|",ph));pk.Sort(StringComparer.Ordinal);ph.Sort(StringComparer.Ordinal);sortedTokenPrefixKey=new SqlString(String.Join("|",pk));sortedTokenPhoneticKey=new SqlString(String.Join("|",ph));
        } catch { }
    }
    ...
    static string ReduceFraction(string value){string[] p=value.Split('/');if(p.Length!=2)return value;int n,d;if(!Int32.TryParse(p[0],out n)||!Int32.TryParse(p[1],out d)||n<1||d<1)return value;int a=n,b=d;while(b!=0){int t=a%b;a=b;b=t;}return (n/a).ToString()+"/"+(d/a).ToString();}
