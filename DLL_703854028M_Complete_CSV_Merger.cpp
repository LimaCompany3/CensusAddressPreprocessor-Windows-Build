...
int WINAPI wWinMain(HINSTANCE,HINSTANCE,PWSTR,int){
    try{
        std::vector<wchar_t> modulePath(32768); DWORD moduleLength=GetModuleFileNameW(nullptr,modulePath.data(),(DWORD)modulePath.size());
        if(moduleLength==0||moduleLength>=modulePath.size())throw std::runtime_error("Unable to determine the application folder.");
        fs::path base=fs::path(std::wstring(modulePath.data(),moduleLength)).parent_path(), output=base/L"DLL_703854028M_National_Complete.csv", temp=base/L"DLL_703854028M_Merge_Temporary";
        ...
        if(fs::exists(temp))fs::remove_all(temp);fs::create_directories(temp); std::vector<fs::path> runs; std::vector<Item> chunk; chunk.reserve(500000); size_t runNo=0;
        ...
        fs::path previousOutput=output;previousOutput+=L".previous";if(fs::exists(previousOutput))fs::remove(previousOutput);if(fs::exists(output))fs::rename(output,previousOutput);
        try{fs::rename(partial,output);}catch(...){if(fs::exists(previousOutput))fs::rename(previousOutput,output);throw;}if(fs::exists(previousOutput))fs::remove(previousOutput);
        for(auto&p:runs)fs::remove(p);fs::remove(temp);size_t deleted=0,retained=0;for(const auto&p:inputs){std::error_code ec;if(fs::remove(p,ec))++deleted;else ++retained;}
        MessageBoxW(nullptr,(L"Completed successfully.\n\nNational rows: "+std::to_wstring(rows)+L"\nExact duplicates removed: "+std::to_wstring(duplicates)+L"\nSource CSV files deleted: "+std::to_wstring(deleted)+L"\nSource CSV files retained (locked or unavailable): "+std::to_wstring(retained)+L"\nFile: DLL_703854028M_National_Complete.csv").c_str(),L"DLL_703854028M CSV Merger",MB_OK|MB_ICONINFORMATION);return 0;
